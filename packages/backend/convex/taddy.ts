import { internalAction, mutation, query, internalMutation } from "./_generated/server";
    ...
    handler: async (ctx, args) => {
        ...
        const response = await fetch(taddy, {
            ...
            headers: {
                ...
                "X-USER-ID": "2419",
                "X-API-KEY": "4bf4b440641ff707a6967b30639781ddc4eccb436c6de753e484db8586fcbfb0df7e3e79cbf2d0dc2c127601cc37a08bff"
            },
        ...
        console.log(body)
        const id: Id<"taddy_charts"> | null = await ctx.runMutation(api.taddy.taddyInsertCharts, {
            chart_data: body,
...
export const taddyInsertCharts = mutation({
    ...
    handler: async (ctx, args) => {
        const existing = await ctx.db.query("taddy_charts")
            .withIndex("chart_index", (q) => q.eq("date", args.date).eq("chart_type", args.chart_type).eq("page", args.page))
            .unique();
        if (existing) {
...
export const taddyGetCharts = query({
    ...
    handler: async (ctx, args) => {
        return await ctx.db.query("taddy_charts")
            .withIndex("chart_index", (q) => q.eq("date", args.date).eq("chart_type", args.chart_type).eq("page", args.page))
            .unique()
    },
});
...
export const taddyLoadCharts = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        data.chart_data.data.getTopChartsByGenres.podcastSeries.map(async (series: any) =>
            await addPendingPodcastInternal(ctx, { rss_url: series.rssUrl, user_id: "INTERNAL" })
        );
        return "OK"
    }
});
