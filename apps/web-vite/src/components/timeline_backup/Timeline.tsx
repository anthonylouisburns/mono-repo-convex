...
export default function Timeline() {
  ...
  const [selectedPodcast, setSelectedPod] = useState<Id<"podcast"> | null>(
    null,
  );
  const [selectedEpisode, setSelectedEpisode] = useState<Id<"episode"> | null>(
    null,
  );
  ...
  function spanDisplay(
    ...
    const podView = podcast ? (
      <div>
        <Link to={"../episodes/" + podcast._id} className="linkStyle1">
          {podcast.title}
        </Link>
      </div>
    ) : (
    ...
    return (
      <div className="boxStyle">
        <div
          ...
          onClick={() => selectSpan(span, episode)}
        >
          {timedisplay(span.start)} to {timedisplay(span.end)}{" "}
          <b>{span.name}</b>
        </div>
        {podView}
        {episodeView}
  ...
  const editSpansView = selectedPodcast ? (
    <EditSpans
      podcast_id={selectedPodcast}
      episode_id={selectedEpisode ? selectedEpisode : undefined}
    />
  ) : (
    ""
  );
  return (
    <div>
      <div className="pagePadding">
        ...
        >
          ...
          {podcasts &&
            podcasts.map((podcast) => (
              <option id={podcast._id} value={podcast._id} key={podcast._id}>
                {podcast.title}
              </option>
            ))}
        </select>
        ...
        {spans &&
          spans.map(({ span, episode, podcast }) => (
            <div key={span._id}>{spanDisplay(span, episode, podcast)}</div>
          ))}
      </div>
    </div>
  );
