...
export const TimeSpans = function TimeSpans({
  spans,
  podcast_id,
  episode_id,
}: {
  ...
  function validDate(dateString: string) {
    ...
    if (
      !(
        dateString.length == 8 ||
        (dateString[0] == "-" && dateString.length == 9)
      )
    ) {
    ...
    if (
      Number.parseInt(dateString.slice(-2)) > 31 ||
      Number.parseInt(dateString.slice(-4, -2)) > 12
    ) {
  ...
  const spanDivs = spans.map((span, index) => {
    return (
      <div key={spans[index]._id}>
        <button className="navigation-button" onClick={() => deleteSpan(index)}>
          -
        </button>
        {timedisplay(span.start)} to {timedisplay(span.end)} {span.name}
      </div>
    );
  });
  return (
    <div>
      <button className="navigation-button" onClick={() => add()}>
        +
      </button>
      <input
