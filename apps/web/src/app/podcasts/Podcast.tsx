...
export const Podcast = function Podcast({ podcast, setName, setRss }: { podcast: Doc<"podcast">, podcastColor: string, setName: Dispatch<SetStateAction<string>>, setRss: Dispatch<SetStateAction<string>> }): JSX.Element {
    ...
    return <div className="boxStyle">
        ...
        }}>{podcast.title ? podcast.title : "title not set"}</Link>
        <button className='navigation-button' onClick={() => updatePodcastRssData({id:podcast._id})}>{podcast.number_of_episodes || 0} episodes</button>
        <a style={redStyle} onClick={() => {
