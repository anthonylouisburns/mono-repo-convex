...
const AccordionPage3 = ({ first, last, expandedPanel, updateExpandedPanel, pageInfo, index, selectedOffset }:
    ...
    const color = pageInfo[2].color[index % pageInfo[2].color.length];
    return (
        <List.Accordion
        ...
        >
            <TimelineLevel4 pageInfo={pageInfo} offset={first.offset} count={last.offset - first.offset} selectedOffset={selectedOffset} />
        </List.Accordion>

    );
};

export default function TimelineLevel3({ pageInfo, offset, count, selectedOffset }: { pageInfo: Array<{ pageSize: number, color: Array<string> }>, offset: number, count: number, selectedOffset: number }) {
    const [expandedPanel, setExpandedPanel] = useState("");
    const result = useQuery(api.page_timeline.getBookmarks, {
        pageSize: pageInfo[2].pageSize * pageInfo[3].pageSize,
        offset: offset,
        count: count
    });
    ...
    return (
        <List.Section>
            {pairs.map((tl, index) => <AccordionPage3 key={index} first={tl.first} last={tl.last} expandedPanel={expandedPanel} updateExpandedPanel={updateExpandedPanel} pageInfo={pageInfo} index={index} selectedOffset={selectedOffset} />)}
        </List.Section>
    );
};